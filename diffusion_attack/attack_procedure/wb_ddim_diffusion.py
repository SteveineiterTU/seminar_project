...
def optimize_z_lbfgs(loss_model, init_val, query_imgs, save_dir, max_func):
    ### store results
    all_loss = []
    all_z = []
    all_x_hat = []
    ...
    for i in tqdm(range(size // BATCH_SIZE)):
        ...
        try:
            ...
            if os.path.exists(save_dir_batch):
                ...
                ### initialize z
                z = Variable(
                    torch.FloatTensor(init_val[i * BATCH_SIZE : (i + 1) * BATCH_SIZE]),
                ).cuda()
                z_model = LatentZ(z)
                ...
                def closure():
                    ...
                    return Variable(final_loss, requires_grad=True)  # TODO ask Ozan if that's fine.
                ...
                for step in range(max_func):
                    ...
                    if step == 0:
                        ...
                        visualize_progress(
                            x_hat_curr, vec_loss_curr, save_dir_batch, step
                        )  # visualize init
                    ...
                    if step == max_func - 1:
                        ...
                        save_files(
                            save_dir_batch, ["l2", "lpips"], [loss_l2, loss_lpips]
                        )
...
def main():
    ...
    query_loss, query_z, query_xhat = optimize_z_lbfgs(
        loss_model,
        init_val_pos,
        pos_query_imgs,
